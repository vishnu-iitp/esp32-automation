...
serve(async (req) => {
  ...
  try {
    ...
    const { data: existingUnclaimed } = await supabaseClient
    ...
    const { error: unclaimedError } = await supabaseClient
      .from('unclaimed_devices')
      .insert({
    ...
    if (unclaimedError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to register unclaimed device' }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
