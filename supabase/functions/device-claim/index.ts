...
serve(async (req)=>{
  ...
  try {
    ...
    const supabaseClient = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_ANON_KEY') ?? '', authHeader ? {
      global: {
        headers: {
    ...
    const { data: device, error: findError } = await supabaseClient.from('devices').select('id, user_id').eq('mac_address', normalizedMac).maybeSingle();
    if (findError) {
    ...
    if (!device) {
      return new Response(JSON.stringify({
        error: 'Device not found. Make sure it is powered on and connected to WiFi.'
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({
        error: 'Failed to claim the device: ' + updateError.message
      }), {
        status: 500,
        headers: {
          ...corsHeaders,
    ...
    const { error: deleteError } = await supabaseClient.from('unclaimed_devices').delete().eq('mac_address', normalizedMac);
    if (deleteError) {
